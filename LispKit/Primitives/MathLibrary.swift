...
public final class MathLibrary: Library {
  ...
  public override func export() {
    define("number?", Procedure(isNumber))
    define("real?", Procedure(isReal))
    define("integer?", Procedure(isInteger))
    define("rational?", Procedure(isRational))
    define("complex?", Procedure(isComplex))
    define("exact?", Procedure(isExact))
    define("inexact?", Procedure(isInexact))
    define("positive?", Procedure(isPositive))
    define("negative?", Procedure(isNegative))
    define("zero?", Procedure(isZero))
    define("even?", Procedure(isEven))
    define("odd?", Procedure(isOdd))
    define("exact->inexact", Procedure(exactToInexact))
    define("inexact->exact", Procedure(inexactToExact))
    define("rationalize", Procedure(rationalize))
    define("floor", Procedure(floor))
    define("ceiling", Procedure(ceiling))
    define("truncate", Procedure(truncate))
    define("round", Procedure(round))
    ...
    define("max", Procedure(max))
    define("min", Procedure(min))
    define("abs", Procedure(absolute))
    define("sqrt", Procedure(sqrt))
    define("expt", Procedure(expt))
    define("exp", Procedure(exp))
    define("log", Procedure(log))
    define("sin", Procedure(sin))
    define("cos", Procedure(cos))
    define("tan", Procedure(tan))
    define("asin", Procedure(asin))
    define("acos", Procedure(acos))
    define("atan", Procedure(atan))
    define("number->string", Procedure(numberToString))
    define("string->number", Procedure(stringToNumber))
    define("make-rectangular", Procedure(makeRectangular))
    define("make-polar", Procedure(makePolar))
    define("real-part", Procedure(realPart))
    define("imag-part", Procedure(imagPart))
    define("magnitude", Procedure(magnitude))
    define("angle", Procedure(angle))
    define("numerator", Procedure(numerator))
    define("denominator", Procedure(denominator))
    define("gcd", Procedure(gcd))
    define("lcm", Procedure(lcm))
    define("quotient", Procedure(quotient))
    define("remainder", Procedure(remainder))
    define("modulo", Procedure(modulo))
    define("fx+", Procedure(fxPlus, compileFxPlus))
    define("fx-", Procedure(fxMinus, compileFxMinus))
    define("fx*", Procedure(fxMult, compileFxMult))
    define("fx/", Procedure(fxDiv, compileFxDiv))
  }
  ...
  func isPositive(expr: Expr) throws -> Expr {
    switch expr {
      ...
      case .Complexnum(let num):
        return num.isReal ? .Boolean(num.re >= 0) : .False
      default:
        throw EvalError.TypeError(expr, [.NumberType])
    }
  }
  ...
  func stringToNumber(expr: Expr, _ rad: Expr?) throws -> Expr {
    ...
    let scanner = Scanner(try expr.asStr(), prescan: false)
    scanner.skipSpace()
    ...
    guard scanner.pos.index >= start.index else {
      throw EvalError.TypeError(expr, [.NumberType])
    }
    ...
    switch token.kind {
      ...
      case .COMPLEX:
        return .Complexnum(token.complexVal)
      default:
        throw EvalError.TypeError(expr, [.NumberType])
    }
  }
  ...
  func rationalize(x: Expr, delta: Expr) throws -> Expr {
    ...
    return .Rat(Rational(n, d))
  }
  
  func makeRectangular(re: Expr, _ imag: Expr) throws -> Expr {
    return .Complexnum(Complex(try re.asFloat(coerce: true), try imag.asFloat(coerce: true)))
  }
  
  func makePolar(abs: Expr, _ arg: Expr) throws -> Expr {
    return .Complexnum(Complex(abs: try abs.asFloat(coerce: true),
