...
;;;
;;; A very simple CLOS-like language, embedded in Scheme, with a simple MOP. The features
  ...
  (begin
    ...
    (define (%allocate-instance class nfields)
      (%allocate-instance-internal
        class
        #t
        (lambda args (error "An instance isn't a procedure -- can't apply it."))
        nfields))
    ...
    (define (%allocate-entity class nfields)
      (%allocate-instance-internal
        class
        #f
        (lambda args (error "Tried to call an entity before its proc is set."))
        nfields))

    (define %allocate-instance-internal (undefined))
    (define %instance?                  (undefined))
    (define %instance-class             (undefined))
    (define %set-instance-class-to-self (undefined))
    (define %set-instance-proc!         (undefined))
    (define %instance-ref               (undefined))
    (define %instance-set!              (undefined))

    (letrec ((instances '())
             (get-vector (lambda (closure)
      ...
      (set! %allocate-instance-internal
        (lambda (class lock proc nfields)
          (letrec ((vector (make-vector (+ nfields 3) #f))
                   (closure (lambda args (apply (vector-ref vector 0) args))))
      ...
      (set! %instance-class
        (lambda (closure)
          (cond ((get-vector closure) => (lambda (vec) (vector-ref vec 2)))
                (else (error "Can't get class of non-instance" closure)))))
      (set! %set-instance-class-to-self
        (lambda (closure)
          (cond ((get-vector closure) => (lambda (vec) (vector-set! vec 2 closure)))
                (else (error "Can't set instance class of non-instance" closure)))))
      ...
      (set! %instance-ref
        (lambda (closure index)
          (cond ((get-vector closure) => (lambda (vec) (vector-ref vec (+ index 3))))
                (else (error "Can't %INSTANCE-REF non-instance" closure index)))))
      (set! %instance-set!
        (lambda (closure index new-value)
          (cond ((get-vector closure) => (lambda (vec) (vector-set! vec (+ index 3) new-value)))
                (else (error "Can't %INSTANCE-SET! non-instance" closure index new-value))))))
        ...
        ((or (eq? class <class>) (eq? class <entity-class>))
    	      (let* ((new (%allocate-instance class (length the-slots-of-a-class)))
    	           ...
    	           (cpl (let loop ((sups dsupers) (so-far (list new)))
    	                  (if (null? sups)
    	                      (reverse so-far)
    	                      (loop (class-direct-supers (car sups)) (cons (car sups) so-far)))))
    		         ...
    		         (getters-n-setters
    		           (map (lambda (s) (cons (car s) (allocator (lambda () '())))) slots)))
    ...
    (define (lookup-slot-info class slot-name)
      (let* ((getters-n-setters (if (eq? class <class>)           ;* This grounds out
    		                            getters-n-setters-for-class   ;* the slot-ref tower.
    		                            (slot-ref class 'getters-n-setters)))
    		     (entry (assq slot-name getters-n-setters)))
        (if entry (cdr entry) (error "No slot" slot-name "in instances of" class))))
    ...
    (define (class-cpl class) (slot-ref class 'cpl))

    (define (generic-methods generic) (slot-ref generic 'methods))

    (define (method-specializers method) (slot-ref method 'specializers))
    (define (method-procedure method) (slot-ref method 'procedure))
    ...
    (define getters-n-setters-for-class   ; see lookup-slot-info
      ...
      (let ((make-em (lambda (s f)
    		     (list s (lambda (o) (%instance-ref  o f))
    			         (lambda (o n) (%instance-set! o f n))))))
        (map (lambda (s) (make-em s (position-of s the-slots-of-a-class)))
    ...
    (define <top> (make <class> 'direct-supers (list) 'direct-slots (list)))

    (define <object> (make <class> 'direct-supers (list <top>) 'direct-slots  (list)))
    ...
    (slot-set! <class> 'field-initializers (map (lambda (s) (lambda () '())) the-slots-of-a-class))
    (slot-set! <class> 'getters-n-setters  '())


    (define <procedure-class> (make <class> 'direct-supers (list <class>) 'direct-slots (list)))

    (define <entity-class> (make <class> 'direct-supers (list <procedure-class>)
                                         'direct-slots  (list)))

    (define <generic> (make <entity-class> 'direct-supers (list <object>)
                                           'direct-slots  (list 'methods)))

    (define <method> (make <class> 'direct-supers (list <object>)
    ...
    (define (make-generic) (make <generic>))

    (define (make-method specializers procedure)
      (make <method> 'specializers specializers 'procedure procedure))
                    ...
                    ;  So here's the fix.]
                       (let loop ((specs (method-specializers method))
                         ...
                         (cond ((null? args) (null? specs))
                               ((null? specs) #t)
                               ((applicable? (car specs) (car args)) (loop (cdr specs) (cdr args)))
                               (else #f))))
    ...
    (add-method initialize
      (make-method (list <object>) (lambda (call-next-method object initargs) object)))
    ...
    (add-method initialize
      (make-method
        ...
        (lambda (call-next-method class initargs)
          (call-next-method)
          (slot-set! class 'direct-supers (getl initargs 'direct-supers '()))
          (slot-set! class 'direct-slots
            (map (lambda (s) (if (pair? s) s (list s))) (getl initargs 'direct-slots '())))
    ...
    (add-method initialize
                (make-method (list <generic>) (lambda (call-next-method generic initargs)
    	            (call-next-method)
    ...
    (add-method compute-cpl
      (make-method
        (list <class>)
        (lambda (call-next-method class) (compute-std-cpl class class-direct-supers))))
    ...
    (define <primitive-class> (make <class> 'direct-supers (list <class>) 'direct-slots (list)))
