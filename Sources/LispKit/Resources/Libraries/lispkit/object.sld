;;; LISPKIT OBJECT
;;;
;;; This library implements a simple, delegation-based object system for LispKit. It
;;; provides procedural and declarative interfaces for objects and classes. The class
;;; system is optional. It mostly provides means to define and manage new object types
;;; and construct objects using object constructors.
;;;
;;; Similar to other Scheme and Lisp-based object systems, methods of objects are defined
;;; in terms of object/class-specific specializations of generic procedures. A generic
;;; procedure consists of methods for the various objects/classes it supports. A generic
;;; procedure performs a dynamic dispatch on the first parameter (the `self` parameter) to
;;; determine the applicable method.
;;;
;;; GENERIC PROCEDURES
;;; 
;;; Generic procedures can be defined using the `define-generic` form. Here is an example
;;; which defines three generic methods, one with only a `self` parameter, and two with
;;; three parameters `self`, `x` and `y`. The last generic procedure definition includes
;;; a `default` method which is applicable to all objects for which there is no specific
;;; method. When a generic procedure without default is applied to an object that does not
;;; define its own method implementation, an error gets signaled.
;;;
...
;;; OBJECTS
;;;
;;; An object encapsulates a list of methods each implementing a generic procedure. These
;;; methods are regular closures which can share mutable state. Objects do not have an
;;; explicit notion of a field or slot as in other Scheme or Lisp-based object systems.
;;; Fields/slots need to be implemented via generic procedures and method implementations
;;; sharing state. Here is an example explaining this approach:
;;;
...
;;;
;;; This is a function creating new point objects. The `x` and `y` parameters of the
;;; constructor function are used for representing the state of the point object. The
;;; created point objects implement three generic procedures: `point-coordinates`,
;;; `set-point-coordinates`, and `object->string`. The latter procedure is defined
;;; directly by the library and, in general, used for creating a string representation
;;; of any object. By implementing the `object->string` method, the behavior gets
...
;;; INHERITANCE
;;;
;;; The LispKit object system supports inheritance via delegation. The following code
;;; shows how colored points can be implemented by delegating all point functionality
;;; to the previous implementation and by simply adding only color-related logic.
;;;
...
;;;
;;; The object created in function `make-colored-point` inherits all methods from object
;;; `super` which gets set to a new point object. It adds a new method to generic
;;; procedure `point-color` and redefines the `object->string` method. The redefinition is
;;; implemented in terms of the inherited `object->string` method for points. The form
;;; `invoke` can be used to refer to overridden methods in delegatee objects. Thus,
;;; `(invoke (super object->string) self)` calls the `object->string` method of the `super`
;;; object but with the identity (`self`) of the colored point.
;;;
...
;;;
;;; Objects can delegate functionality to multiple delegatees. The order in which they are
;;; listed determines the methods which are being inherited in case there are conflicts,
;;; i.e. multiple delegatees implement a method for the same generic procedure.
;;;
;;; CLASSES
;;;
;;; Classes add syntactic sugar, simplying the creation and management of objects. They
;;; play the following role in the object-system of LispKit:
;;;
...
;;;   3) A class can inherit functionality from several other classes, making it easy
...
;;;
;;; Each object created by a class implements a generic procedure `object-class` referring
;;; to the class of the object. Since classes are objects themselves we can obtain their
;;; name with generic procedure `class-name`:
;;;
...
;;;
;;; Generic procedure `instance-of?` can be used to determine whether an object is a
;;; direct or indirect instance of a given class. The last two lines above show that
;;; `pt2` is an instance of `point`, but `pt` is not, even though it is functionally
;;; equivalent.
;;;
;;; The following definition re-implements the colored point example from above using
;;; a class:
;;;
...
;;;         (string-append (object->string color) ":" (invoke (super object->string) self)))))
;;;
;;; The following lines illustrate the behavior of `colored-point` objects vs `point`
;;; objects:
;;;
