  ...
  (begin
    ...
    (define-syntax for
      (syntax-rules (in from to step where)
        ((_ (i element) in lst where condition body ...)
           (if (not (null? lst))
               (do ((i 0 (+ i 1))
                    (xs lst (cdr xs))
                    (element (car lst) (and (pair? (cdr xs)) (cadr xs))))
                   ((null? xs))
                 (if condition (begin body ...)))))
        ((_ element in lst where condition body ...)
           (dolist (element lst) (if condition (begin body ...))))
        ((_ (i element) in lst body ...)
           (if (not (null? lst))
               (do ((i 0 (+ i 1))
                    (xs lst (cdr xs))
                    (element (car lst) (and (pair? (cdr xs)) (cadr xs))))
        ...
        ((_ (i element) from (x ...) body ...)
           (for (i element) in (list x ...) body ...))
