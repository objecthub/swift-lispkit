;;; LISPKIT WT-TREE
;;;
;;; This is the standard implementation of weight-balanced binary trees as provided
;;; by MIT-Scheme.
;;;
;;; Original source:
;;; "wttree.scm" Weight balanced trees
;;;
...
(define-library (lispkit wt-tree)
  ...
  (begin
    ...
    (define (t-join k v l r)
      ...
      (let ((l_n (fx+ (node/size l) 1))
            (r_n (fx+ (node/size r) 1)))
        (cond ((fx> r_n (fx* wt-tree-delta l_n))
               ;; right is too big
               (let ((r_l_n (fx+ (node/size (node/l r)) 1))
                     (r_r_n (fx+ (node/size (node/r r)) 1)))
                 (if (fx< r_l_n (fx* wt-tree-gamma r_r_n))
                   (single-l k v l r)
                   (double-l k v l r))))
              ((fx> l_n (fx* wt-tree-delta r_n))
               ;; left is too big
               (let ((l_l_n (fx+ (node/size (node/l l)) 1))
                     (l_r_n (fx+ (node/size (node/r l)) 1)))
      ...
      (define (loop node index)
        (let ((size_l  (node/size (node/l node))))
          (cond ((fx< index size_l)  (loop (node/l node) index))
                ((fx> index size_l)  (loop (node/r node)
                                             (fx- index (fx+ 1 size_l))))
                (else                  node))))
    ...
    (define number-wt-type (local:make-wt-tree-type  <))
