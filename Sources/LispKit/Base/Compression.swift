...
//
//  Based on DataCompression by Markus Wanke found at:
...
public extension Data {
  ...
  enum CompressionAlgorithm {
    ...
    /// Deflate: Fast with a good compression rate. Proved itself over time and is supported
    /// everywhere.
    case zlib

    /// Apples custom Lempel-Ziv style compression algorithm. Claims to compress as good as zlib
    /// but 2 to 3 times faster.
    case lzfse

    /// Horribly slow. Compression as well as decompression. Compresses better than zlib though.
    case lzma

    /// Fast, but compression rate is very bad. Apples lz4 implementation often do not
    /// compress at all.
    case lz4

    fileprivate var lowLevelType: compression_algorithm {
      switch self {
  ...
  func zip() -> Data? {
    ...
    guard var result = deflated else {
      return nil
    }
  ...
  func gunzip() -> Data? {
    ...
    let cresult: Data? = withUnsafeBytes { (ptr: UnsafePointer<UInt8>) -> Data? in
      ...
      guard pos < limit else {
        return nil
      }
      let config = (operation: COMPRESSION_STREAM_DECODE, algorithm: COMPRESSION_ZLIB)
      return perform(config, source: ptr.advanced(by: pos), sourceSize: limit - pos)
    }
    guard let inflated = cresult,
          ftr.isize == UInt32(truncatingIfNeeded: inflated.count),
          ftr.crc32 == inflated.crc32().checksum else {
      return nil
    }
    return inflated
  }
...
public struct Crc32: CustomStringConvertible {
  ...
  private(set) public var checksum: UInt32 = 0
  ...
  private static func loadCrc32fromZLib() -> ZLibCrc32FuncPtr? {
    guard let libz = dlopen("/usr/lib/libz.dylib", RTLD_NOW),
          let fptr = dlsym(libz, "crc32") else {
      return nil
    }
...
public struct Adler32: CustomStringConvertible {
  ...
  /// Raw checksum. Updated after a every call to `advance(withChunk:)`
  private(set) public var checksum: UInt32 = 1
  ...
  private static func loadAdler32fromZLib() -> ZLibAdler32FuncPtr? {
    guard let libz = dlopen("/usr/lib/libz.dylib", RTLD_NOW),
          let fptr = dlsym(libz, "adler32") else {
      return nil
    }
...
fileprivate extension Data {
  func withUnsafeBytes<ResultType, ContentType>(
         _ body: (UnsafePointer<ContentType>) throws -> ResultType) rethrows -> ResultType {
...
fileprivate typealias Config = (operation: compression_stream_operation,
                                algorithm: compression_algorithm)

fileprivate func perform(_ config: Config,
  ...
  if sourceSize > blockLimit {
    ...
    if config.algorithm == COMPRESSION_LZFSE && config.operation != COMPRESSION_STREAM_ENCODE   {
      // This fixes a bug in Apples lzfse decompressor. it will sometimes fail randomly when the
      // input gets splitted into multiple chunks and the flag is not 0. Even though it should
      // always work with FINALIZE...
      flags = 0
    }
  }
