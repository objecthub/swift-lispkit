...
public final class SystemLibrary: NativeLibrary {
  ...
  private func urlScheme(_ expr: Expr) throws -> Expr {
    if let res = URL(string: try expr.asString())?.scheme {
  ...
  private func urlHost(_ expr: Expr, _ percentEncoded: Expr?) throws -> Expr {
    if let url = URL(string: try expr.asString()) {
      if let percentEncoded {
        if let res = url.host(percentEncoded: percentEncoded.isTrue) {
          return .makeString(res)
        } else {
          return .false
        }
      } else if let res = url.host {
        return .makeString(res)
      } else {
        return .false
      }
  ...
  private func urlPort(_ expr: Expr) throws -> Expr {
    if let res = URL(string: try expr.asString())?.port {
  ...
  private func fileUrl(_ expr: Expr, _ relativeTo: Expr?, _ expand: Expr?) throws -> Expr {
    ...
    if let relativeTo, relativeTo.isTrue {
      if let root =  URL(string: try relativeTo.asString()), root.isFileURL {
  ...
  private func isFileUrl(_ expr: Expr) throws -> Expr {
    guard case .string(let str) = expr,
          let url = URL(string: str as String),
          url.isFileURL else {
      return .false
